...
/* Handler: READY */
static void on_cmd_atcmdecho_nosock_ready(struct net_buf **buf, u16_t len)
{
    LOG_INF("READY");
}
...
restart:
	...
	ret = send_at_cmd(NULL, "AT+CPIN?", MDM_CMD_TIMEOUT);
	if (ret < 0) {
		LOG_ERR("AT+CPIN? ret:%d", ret);
		goto error;
	}
	...
	ret = send_at_cmd(NULL, "AT+COPS?", MDM_CMD_TIMEOUT);
	if (ret < 0) {
    ...
    ret = send_at_cmd(NULL, QICSGP_CHINA_UNICOM, K_SECONDS(2));
    if (ret < 0) {
        LOG_ERR("AT+QICSGP ret:%d", ret);
    }
    */
